...
struct Material{
    sampler2D diff_map, emit_map, spec_map;
...
void main(){
    vec4 emit = texture(material.emit_map, tex_coord);
    emit *= material.emit_strength;

    FragInfo frag_info;
    frag_info.n_normal = normalize(normal);
    frag_info.n_view_dir = normalize(frag_pos - cam_pos);
    frag_info.diff_map = texture(material.diff_map, tex_coord).rgb;
    frag_info.spec_map = texture(material.spec_map, tex_coord).rgb;
    ...
    frag_color = emit + vec4(light_output, 1.0);
}
